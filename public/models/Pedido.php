...
class Pedido
{
    ...
    public function crearPedido()
    {
        ...
        $consulta = $objAccesoDatos->prepararConsulta("INSERT INTO pedidos (id_mesa, id_producto, cantidad, cliente, codigo, id_estado_pedido, fecha, id_empleado,nombre_foto ) VALUES (:id_mesa, :id_producto, :cantidad, :cliente, :codigo, :id_estado_pedido, :fecha, :id_empleado, :nombre_foto)");
    ...
    public static function ObtenerTiempoDePedido($numeroPedido, $codigoMesa){

        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("SELECT pedidos.cantidad as cantidad, productos.nombre as productos, pedidos.tiempo as demora FROM pedidos INNER JOIN mesas INNER JOIN productos ON pedidos.codigo = ? AND pedidos.id_mesa = mesas.id AND mesas.codigo = ? AND pedidos.id_producto = productos.id
        ");
        $consulta->execute(array($numeroPedido,$codigoMesa));
        return $consulta->fetchAll(PDO::FETCH_ASSOC);
        /*0708  ertye*/
    }
